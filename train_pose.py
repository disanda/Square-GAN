...
#--------training D-----------
	        ...
	        r_loss = torch.max( torch.randn(1).to(device) - x_real_d_logit.to('cpu'), torch.zeros_like(x_real_d_logit)).mean() #shift_randomD 
	        f_loss = torch.max( torch.randn(1).to(device) + x_real_d_logit.to('cpu'), torch.zeros_like(x_real_d_logit)).mean()
