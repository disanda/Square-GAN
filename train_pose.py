...
#--------training D-----------
	        ...
	        r_loss = torch.max( torch.randn(1).to(device) - x_real_d_logit, torch.zeros_like(x_real_d_logit)).mean() #shift_randomD 
	        f_loss = torch.max( torch.randn(1).to(device) + x_fake_d_logit, torch.zeros_like(x_fake_d_logit)).mean()
...
#-----------training G-----------
	        ...
	        G_loss = torch.max( torch.randn(1).to(device)-x_fake_d_logit_2, torch.zeros_like(x_fake_d_logit_2) ).mean() #* ((args.epochs-ep)//args.epochs) ) #渐进式loss
	        G.zero_grad()
	        G_loss.backward()
	        G_optimizer.step()
	        it_g += 1
