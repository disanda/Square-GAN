...
#--------training D-----------
	        ...
	        r_loss = torch.max( 0.5+torch.randn(1).to(device) - x_real_d_logit, torch.zeros_like(x_real_d_logit)).mean() #shift_randomD 
	        f_loss = torch.max( 0.5+torch.randn(1).to(device) + x_fake_d_logit, torch.zeros_like(x_fake_d_logit)).mean()
