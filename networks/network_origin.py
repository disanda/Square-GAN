...
class ConvGenerator(nn.Module):
    def __init__(self,
        ...
        layers.append(dconv_norm_relu(input_dim, d, kernel_size=4, stride=1, padding=0, biass = biaS))
        # 2: upsamplings, 4x4 -> 8x8 -> 16x16 -> ...
        for i in range(n_upsamplings - 1):
            d_last = d
            d = d//2
            layers.append(dconv_norm_relu(d_last, d, kernel_size=4, stride=2, padding=1,biass=biaS))
        layers.append(nn.ConvTranspose2d(d, output_channels, kernel_size=4, stride=2, padding=1, bias=biaS))
...
class ConvDiscriminator(nn.Module):
    def __init__(self,
        ...
        d = dim
        layers.append(nn.Conv2d(input_channels, d, kernel_size=4, stride=2, padding=1, bias=biaS))
        layers.append(nn.LeakyReLU(0.2))
        for i in range(n_downsamplings - 1):
            d_last = d
            d = d*2 # 1 > 2 > 4 > 8  > 16 (64*64) > 32 > 64 >128 > 256 (1024*1024) 
            layers.append(conv_norm_lrelu(d_last, d, kernel_size=4, stride=2, padding=1, biass=biaS))
        # 2: logit
        layers.append(nn.Conv2d(d, 1, kernel_size=4, stride=1, padding=0, bias=biaS))
        self.net = nn.Sequential(*layers)
    def forward(self, x):
        y = self.net(x)
        return y # [1,1,1,1] 
